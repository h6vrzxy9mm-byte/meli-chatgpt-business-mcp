...
export function registerTools(server: McpServer, client: MercadoLibreClient) {
  ...
  const draftSchema = { title: z.string().min(3).max(200), category_id: z.string(), price: z.number().positive(), stock: z.number().int().positive(), listing_type_id: z.enum(['gold_special', 'gold_pro']), free_shipping: z.boolean().default(false), pictures: z.array(z.string().url()).min(1), attributes: z.array(z.object({ id: z.string(), value_name: z.string().optional(), value_id: z.string().optional() })).default([]), description: z.string().optional(), family_name: z.string().optional(), product_cost: z.number().nonnegative().optional(), dimensions: z.string().optional(), logistic_type: z.string().optional() };
  server.registerTool('meli_prepare_listing', { description: 'Genera una vista previa completa BORRADOR - NO PUBLICADO y valida atributos/costos.', inputSchema: draftSchema, annotations: readOnly }, async (a) => {
